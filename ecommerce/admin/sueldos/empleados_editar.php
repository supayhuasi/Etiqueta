...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['action']) && $_POST['action'] === 'set_sueldo_mensual') {
            ...
            header("Location: ../empleados_editar.php?id=" . urlencode($empleado_id) . "&success=Empleado actualizado");
            exit;
        }
    } catch (Exception $e) {
...
<?php
header('Location: ../empleados_editar.php' . (isset($_GET['id']) ? '?id=' . urlencode($_GET['id']) : ''));
exit;
?>
