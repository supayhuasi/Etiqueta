        ...
        .then(data => {
            ...
            if (data.atributos && data.atributos.length > 0) {
                ...
                data.atributos.forEach(attr => {
                    ...
                    if (attr.tipo === 'text') {
                        inputHTML = `<input type="text" class="form-control form-control-sm mb-2" data-attr-valor="${attr.id}" ${requerido} oninput="actualizarCostoAtributo(${attr.id}, ${attr.costo_adicional}, 0, this.value)">`;
                    } else if (attr.tipo === 'number') {
                        inputHTML = `<input type="number" class="form-control form-control-sm mb-2" data-attr-valor="${attr.id}" step="0.01" ${requerido} oninput="actualizarCostoAtributo(${attr.id}, ${attr.costo_adicional}, 0, this.value)">`;
                    } else if (attr.tipo === 'color') {
                        const inputId = `modal_attr_${attr.id}`;
                        const previewId = `modal_color_preview_${attr.id}`;
                        inputHTML = `
                            <div class="d-flex align-items-center gap-2 mb-2">
                                <input type="color" class="form-control form-control-color" id="${inputId}" data-attr-valor="${attr.id}" value="#000000" ${requerido} oninput="actualizarCostoAtributo(${attr.id}, ${attr.costo_adicional}, 0, this.value)">
                                    ...
                                    return `
                                        <div class="position-relative">
                                            <label class="cursor-pointer position-relative" style="cursor: pointer;">
                                                <input type="radio" name="modal_attr_${attr.id}" value="${o.valor}" class="d-none attr-radio" data-attr-id="${attr.id}" data-costo="${o.costo || 0}" ${requerido} onchange="actualizarCostoAtributo(${attr.id}, ${attr.costo_adicional}, this.dataset.costo, this.value); marcarOpcionAtributo(this);">
