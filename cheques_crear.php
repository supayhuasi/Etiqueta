...
// Si es edición, obtener datos del cheque
if ($id > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id == 0) {
        $stmt = $pdo->prepare("SELECT id FROM cheques WHERE numero_cheque = ?");
    ...
    if (empty($errores)) {
        try {
            ...
            // Recargar datos
            $stmt = $pdo->prepare("SELECT * FROM cheques WHERE id = " . ($id > 0 ? $id : "LAST_INSERT_ID()"));
            $stmt->execute();
            $cheque = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
        ...
        <div class="col-md-8 offset-md-2">
            ...
            <?php if (isset($mensaje)): ?>
                <div class="alert alert-success" role="alert">
                    <?= $mensaje ?>
                    <br><a href="cheques.php" class="btn btn-primary btn-sm mt-2">Volver a Cheques</a>
                </div>
            <?php endif; ?>
                ...
                <div class="card-body">
                    <form method="POST">
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="cheques.php" class="btn btn-secondary">Cancelar</a>
                            <button type="submit" class="btn btn-primary">
                                <?= $id > 0 ? 'Actualizar Cheque' : 'Crear Cheque' ?>
                            </button>
                        </div>
                    </form>
                </div>
