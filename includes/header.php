...
// Para roles ventas/operario, solo permitir acceso al escaneo (ahora unificado)
// El admin puede acceder a cualquier lugar
if (isset($_SESSION['rol']) && in_array($_SESSION['rol'], ['ventas','operario'])) {
    // además de las APIs antiguas dejamos permitidos los nuevos endpoints de escaneo
    $allowed = ['scan.php', 'scan_api.php', 'escanear_asistencia.php', 'escanear_asistencia_procesar.php'];
    $current = basename($_SERVER['PHP_SELF']);
    if (!in_array($current, $allowed)) {
        // dirigir al nuevo punto único de escaneo
