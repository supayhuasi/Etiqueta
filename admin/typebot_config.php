...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($wu !== false) {
    ...
    // Si también se pudo escribir en includes, indicar que se sincronizó
    if ($w_includes !== false) {
    ...
    // Si no se pudo escribir en uploads, intentar escribir en includes aunque improbable
    if ($w_includes !== false) {
...
<div class="container mt-4">
  ...
  <?php if (!empty($raw_json_for_copy)): ?>
    ...
    <div class="mt-2"><strong>Comandos sugeridos (ejecutar en el servidor):</strong>
      <pre>sudo chown -R www-data:www-data <?= dirname(__DIR__) . '/includes/typebot_config.json' ?>
